...
The `quantile(dist, prob)` gives the value x for which the cumulative probability for all values
lower than x is equal to prob. It is the inverse of `cdf`. In the literature, it
is also known as the quantiles function. In the optional `summary statistics` panel beneath a
distribution, the values beneath 5%, 10%, 25%, etc are the quantiles of that distribution 
for the respective percentages.
